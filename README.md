...
| `/donnees` | Données | Catalogue des **38 sources** avec fraîcheur mesurée (le moniteur de santé des sources), licences, règles des alertes, **7** exports JSON statiques (`/api/meta.json`, `alertes`, `elus`, `budget-mensuel`, `marches-agregats`, `lobbying-marches`, `/data/recherche-index.json`) reconstruits à chaque publication. |
...
![Données & exports : 38 sources tracées, moniteur de fraîcheur](docs/screenshots/donnees.png)

## Sources & licences

Sources majeures (le catalogue **complet et daté** des 38 sources est la page `/donnees`, régénérée à chaque publication ; le référentiel : [docs/SOURCES.md](docs/SOURCES.md)) :
