...
- Headers de sécurité : Pages ne permet pas de headers custom → CSP portée par `<meta http-equiv>` (limite connue : `frame-ancestors` ignoré en meta — risque clickjacking résiduel faible, documenté). **Constaté en R3 (curl réels du 19/08/2026)** : Pages ne sert ni `Strict-Transport-Security` ni `X-Content-Type-Options` ; contrairement à la croyance initiale, `github.io` ne figure plus dans la liste de préchargement HSTS de Chromium (vérifié dans `transport_security_state_static.json` amont — seuls github.com/github.blog y sont). Protections réelles : redirection 301 HTTP→HTTPS systématique (vérifiée) + `https_enforced` ; Content-Type explicites corrects sur tous les fichiers (le risque MIME-sniffing est marginal sur un site statique sans contenu utilisateur) ; `Cache-Control: max-age=600` posé par Pages, cohérent avec un site quotidien. Risque résiduel assumé : la toute première visite tapée en `http://` transite une fois en clair avant redirection — levable plus tard via domaine custom + CDN (ACTIONS-HUMAINES.md).
