...
**Constat décisif (vérifié en base le 21/08/2026, table `meta_sources`)** : sur les **30 sources ingérées, 27 sont en Licence Ouverte** (variantes : LO 2.0, fr-lo, etalab-2.0, LO Etalab, LO (AN), LO (IGN/Etalab), mention DILA) — la dernière entrée en date étant le stock Sirene de l'INSEE, en LO 2.0 ; deux sont des entrées « publications officielles / texte JORF » (constantes factuelles issues d'actes officiels, hors droit d'auteur, citées avec URL) ; la dernière, le registre de transparence de l'Union européenne, relève de la **décision 2011/833/UE** sur la réutilisation des documents de la Commission — réutilisation libre, sans partage à l'identique. **Aucune source ODbL n'est en base** — OFGL est en LO 2.0. L'ODbL ne concernerait que des sources non ingérées (HowTheyVote, données Parlement européen ODbL+DbCL, historique NosDéputés). **La SQLite actuelle n'a donc aucune obligation de re-partage.** Corollaire : le footer ne mentionne que la Licence Ouverte, l'ODbL en ayant été retirée comme inexacte (voir § 3).
...
**Registre art. 30** : une fiche de registre des activités de traitement est tenue hors ligne depuis le 21/08/2026, sur le serveur et hors du dépôt. Elle n'est pas publiée — l'article 30 impose de la tenir et de la communiquer à l'autorité de contrôle sur demande (art. 30.4), pas de la rendre publique. Elle couvre quatre traitements : journaux de serveur, republication de données publiques, accès restreint à la restitution d'audience, correspondance reçue. **Elle se reprend à chaque nouvelle source** : les durées de conservation qu'elle porte doivent rester identiques à celles annoncées au visiteur sur `/donnees-personnelles`.
...
- **Registre art. 30 pour une personne physique : zone grise** doctrinale, tranchée par la position la plus sûre et la moins coûteuse — tenir la fiche plutôt que débattre de son caractère obligatoire. Elle est tenue hors ligne depuis le 21/08/2026 et n'est pas publiée (§ 3).
