...
2 bis. **Perte de rattachement HATVP** (`pipelines/ingest_hatvp_declarations.py`) — l'ingestion s'arrête sur `perte de rattachement : N déclaration(s) toujours publiées par la HATVP ne sont plus rattachées`.
   Conséquence : **aucune** — rien n'a été écrit, le site reste sur la veille. Mais l'ingestion étant tout-ou-rien, **les pipelines en aval n'ont pas tourné** : le cycle entier est à relancer.
   ...
   ```
   puis **`ft-deploy --sans-ingest`** — et non `ft-deploy` tout court, qui rejouerait l'ingestion complète : `/etc/sudoers` porte `env_reset` et `ft-deploy` appelle les pipelines par `sudo -u ftweb --set-home bash -c`, si bien que **l'acquittement ne le traverse pas** (mesuré : la variable arrive vide). Deux messages voisins relèvent de la même famille : `rubrique effondrée : <rubrique> passe de N lignes à 0` (une balise amont a été renommée hors de la liste blanche `RUBRIQUES` — correctif de pipeline, **jamais** d'acquittement ; ce contrôle-là est délibérément posé AVANT la perte de rattachement, pour qu'un message acquittable ne masque jamais une rupture de lecture qui ne l'est pas), et les avertissements `hatvp_decl_interets existe mais est VIDE` / `hatvp_decl_lignes existe mais est VIDE`, qui signalent que le cycle précédent est mort **en cours d'écriture** : les deux contrôles inter-cycles sont sans mémoire ce cycle-ci et redeviennent opérants au suivant. Corollaire à connaître : **ces deux contrôles ne sont jamais exercés par l'intégration continue**, qui part toujours d'une base neuve — ils ne vivent que sur le serveur.
