...
2 bis. **Perte de rattachement HATVP** (`pipelines/ingest_hatvp_declarations.py`) — l'ingestion s'arrête sur `perte de rattachement : N déclaration(s) toujours publiées par la HATVP ne sont plus rattachées`.
   Conséquence : **aucune** — rien n'a été écrit, le site reste sur la veille. Mais l'ingestion étant tout-ou-rien, **les pipelines en aval n'ont pas tourné** : le cycle entier est à relancer — en DEUX temps : d'abord P15 seul avec l'acquittement (ci-dessous), puis `ft-deploy` complet, qui rejoue les 31 pipelines.
   Ce que ça veut dire : une déclaration d'intérêts que la HATVP publie **toujours** a cessé d'être rattachée à un élu qui porte **encore** une fiche. Ce n'est ni un retrait amont ni une fin de mandat — ces deux cas-là passent sans bruit. C'est presque toujours l'état civil d'un élu qui a changé dans `elus` (P7/P9 réécrivent nom, prénom et date de naissance à chaque cycle), ou un homonyme de la même année qui vient d'y entrer et fait renoncer le repli d'orthographe.
   Conduite : le message nomme **au plus huit** élus et huit uuid, et le dit par un « … ». La liste complète part sur la **sortie d'erreur du cycle** — ligne `perte de rattachement, liste complète (N) : …` : à l'écran si vous avez lancé `ft-deploy` à la main, dans `deploiement.log` si c'est la minuterie qui l'a lancé. ⚠️ **Un `ft-deploy` lancé à la main n'écrit PAS la sortie des pipelines dans le journal** — seule l'unité systemd redirige `StandardError` vers le fichier, et 40 des 44 cycles enregistrés n'y portent aucune ligne de pipeline. Ne fermez pas le terminal avant d'avoir relevé la liste. Ouvrir ensuite une ou deux fiches concernées sur `/elus/<id>` et comparer à `https://www.hatvp.fr` : si l'appariement doit être réparé, c'est un correctif de pipeline (proposition de fusion, la CI vérifiera). 🛑 **L'acquittement n'est PAS une réparation : c'est un interrupteur de perte de données, et il est irréversible.** Un uuid acquitté n'est pas écrit en base ; au cycle suivant il ne figure plus dans l'état précédent, et le contrôle se taira sur lui **pour toujours**. Ne l'employer que si la perte est réelle et assumée — l'élu a bien changé d'identité et la déclaration ne lui revient plus :
   ```
   sudo -u ftweb env FT_P15_PERTES_ACQUITTEES='<uuid>,<uuid>' \
     bash -c 'cd /srv/france-transparence/app && make ingest-hatvp_declarations'
   ```
   puis **`ft-deploy` tout court**, et surtout PAS `--sans-ingest`. Le raisonnement, qui a été faux ici jusqu'au 26/08/2026 : il est exact que l'acquittement **ne traverse pas** `ft-deploy` (`/etc/sudoers` porte `env_reset`, `ft-deploy` appelle les pipelines par `sudo -u ftweb --set-home bash -c` ; mesuré : la variable arrive vide) — mais **il n'a plus à le traverser**. La commande ci-dessus a déjà réécrit `hatvp_decl_interets` sans les uuid acquittés : ils ont quitté l'état précédent, le contrôle n'a plus rien à leur reprocher, et le cycle complet passe (mesuré par `test_gardefou_perte_acquittee_une_par_une_laisse_passer`, qui rejoue un second cycle **sans** la variable). ⚠️ `--sans-ingest` ne relancerait **aucun** pipeline (il pose `INGESTION=0` et saute `make ingest`) : le site serait rebâti sur les tables en aval laissées à l'état d'avant l'échec — exactement ce que la ligne « le cycle entier est à relancer » ci-dessus interdit. Deux messages voisins relèvent de la même famille : `rubrique effondrée : <rubrique> passe de N lignes à 0` (une balise amont a été renommée hors de la liste blanche `RUBRIQUES` — correctif de pipeline, **jamais** d'acquittement ; ce contrôle-là est délibérément posé AVANT la perte de rattachement, pour qu'un message acquittable ne masque jamais une rupture de lecture qui ne l'est pas), et les avertissements `hatvp_decl_interets existe mais est VIDE` / `hatvp_decl_lignes existe mais est VIDE` : les deux contrôles inter-cycles sont sans mémoire ce cycle-ci et redeviennent opérants au suivant. ⚠️ **La lecture de ces deux avertissements a changé le 26/08/2026** : ils signalaient jusque-là un cycle mort en cours d'écriture, cause qui n'existe plus — `ecrire()` ouvre une transaction explicite, un cycle qui échoue laisse la base du cycle précédent intacte. Les voir aujourd'hui, c'est que la table a été vidée **hors du pipeline** : restauration partielle, écriture manuelle, migration. Ne pas relancer le cycle en croyant réparer : chercher d'abord ce qui a touché la base. Corollaire à connaître : **ces deux contrôles ne sont jamais exercés par l'intégration continue**, qui part toujours d'une base neuve — ils ne vivent que sur le serveur. 🛑 **Un message que vous ne verrez PAS, et qui coûte plus cher que celui-ci : `… se détachent de N élu(s) qui ne figurent plus parmi les fiches publiées (fin de mandat attendue) — SANS RETOUR`.** C'est un AVERTISSEMENT, pas un échec : le cycle réussit, le site se publie, personne n'est réveillé. Le prix est que ces uuid quittent la mémoire du garde-fou **définitivement**, comme s'ils avaient été acquittés — sauf que personne ne l'a décidé. Si l'élu retrouve sa fiche ensuite (P7/P9 réparé, **renouvellement du Sénat du 27/09/2026**, fusion des doublons `rne-*`) pendant que sa déclaration reste détachée, **plus rien ne le dira jamais**. Des trois voies qui font oublier un uuid, c'est la seule automatique : si vous voyez « SANS RETOUR » dans `deploiement.log`, relevez les uuid le jour même — au cycle suivant ils auront disparu de la base comme du contrôle.
2 ter. **Intégrité des élus en échec** (`pipelines/ingest_integrite.py`, P7) — l'ingestion s'arrête sur n'importe quelle levée de ce pipeline : volumétrie RNE invraisemblable, `liste.csv` sous le seuil de 10 000 lignes, colonnes du CSV des conseillers municipaux qui ont dérivé.
   Conséquence : **aucune sur les données, depuis le 26/08/2026** — l'écriture de P7 est atomique (`appliquer_schema()` ouvre un `BEGIN IMMEDIATE`, le schéma n'est plus appliqué par `executescript`, qui validait implicitement). `hatvp_declarations`, `hatvp_agregats`, `rne_cm_agregats` et les alertes A1 gardent intégralement leur contenu du cycle précédent, et le site reste sur la veille. `elus` n'est de toute façon jamais écrasée, seulement complétée.
   🛑 **Ce que ça valait AVANT le 26/08/2026, et pourquoi la conduite a changé** : les trois `DROP TABLE` du schéma partaient sur disque avant le premier `INSERT`, et le `rollback()` n'annulait que les insertions. Une panne laissait les trois tables **existantes et vides** — mesuré sur une copie de la base servie : **13 277 / 37 / 104 lignes devenaient 0 / 0 / 0**. Le piège n'était pas dans le cycle qui échoue (il ne publie rien) mais dans le geste d'après : **`ft-deploy --sans-ingest` aurait rebâti et PUBLIÉ le site sur ces tables vides**, sans qu'aucun contrôle de santé ne s'y oppose — celui qui compte les fiches (`NB_ELUS ≥ 900`) lit `elus`, que P7 n'efface jamais. Un site à zéro déclaration HATVP passait tous les contrôles.
   Conduite : identifier la levée dans `deploiement.log` ; reproduire avec `make ingest-integrite` ; corriger le pipeline et ses tests, ouvrir une proposition de fusion. **Ne rien restaurer** : la base est saine. `--sans-ingest` est de nouveau sans danger ici, mais reste inutile — il ne relancerait aucun pipeline.
