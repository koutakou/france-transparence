...
État courant : **26 pipelines**, **38 sources**, **11 onglets** + `/comprendre` hors nav, kit `app/src/components/ui/` écrit (G2/G8). Le flux de données et le schéma SQL noyau ci-dessous n'ont pas changé de contrat.
...
CREATE TABLE meta_sources (           -- fraîcheur : donnée de premier rang
    source_id      TEXT PRIMARY KEY,  -- 'S1'…'S47', ids composés (S5-AMO10, S27-*, …)
    nom            TEXT NOT NULL,
    url            TEXT NOT NULL,
