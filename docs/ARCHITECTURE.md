...
État courant : **31 pipelines**, **44 sources**, **11 onglets** + `/comprendre` hors nav, kit `app/src/components/ui/` écrit (G2/G8). Le flux de données et le schéma SQL noyau ci-dessous n'ont pas changé de contrat.
...
CREATE TABLE meta_sources (           -- fraîcheur : donnée de premier rang
    source_id      TEXT PRIMARY KEY,  -- 'S1'…'S52', ids composés (S5-AMO10, S27-*, …)
    nom            TEXT NOT NULL,
    url            TEXT NOT NULL,
...
| `ingest-adlc` | `ingest_adlc.py` | P29 — Sanctions financières de l'Autorité de la concurrence (S52, CSV 2009+ joint aux métadonnées). Aucune dépendance d'ordre : n'écrit que `adlc_decisions` et `adlc_lignes`. Grain = `SUM(Montant total)` une fois par `id_decision`. JSON texte intégral non ingéré. Distinct de S13 (budget) et de S39 (jaune opérateurs). Placé avant `sirene`. |
